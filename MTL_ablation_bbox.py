...
### ablation bbox model
class MTL_bbox(nn.Module):
    
    def __init__(self):
        super(MTL_bbox,self).__init__()
        ...
        self.cross_stitch = nn.Parameter(torch.tensor([[0.8,0.2],[0.2,0.8]]))
    ...
    def forward(self, x):
        ...
        bbox_output = self.bbox_task(torch.flatten(self.adapool(x_1),1))
        seg_output = self.seg_task(x_2)

        return [bbox_output,seg_output]
...
EPOCH = 150
LOSS_PRE_WEIGHT = [0.5, 1.5]

num_batches_train = len(train_loader)
num_batches_val = len(val_loader)

## train: all_loss, bbox_loss, seg_loss, iou_bbox, seg_miou
## val: bbox_loss, seg_loss, iou_bbox, seg_miou
...
for epoch in range(EPOCH):  # loop over the dataset multiple times
    ...
    for index, data in enumerate(train_loader, 0):
        ...
        loss_bbox = Loss_DIoU(outputs[0], labels_bbox, device)
        ...
        if index < 2:
            ...
            temp = torch.cat((running_loss_bbox[index-2:index], running_loss_seg[index-2:index]))
        ...
        if loss_bbox <0.2 :
            ...
            loss = loss_weight[0]*loss_bbox + loss_weight[1]*loss_seg

        loss.backward()
        optimizer.step()

        running_bbox_iou[index] = Metrics_IoU(outputs[0], labels_bbox, device).item()
    ...
    history[epoch,0], history[epoch,1], history[epoch,2] = running_loss_all.mean(), running_loss_bbox.mean(), running_loss_seg.mean()
    history[epoch,3], history[epoch,4] = running_bbox_iou.mean(), running_seg_miou.mean()
    ...
    with torch.no_grad():
        ...
        for index, data in enumerate(val_loader, 0):
            ...
            val_bbox_iou[index] = Metrics_IoU(outputs[0], labels_bbox, device).item()
    ...
    history[epoch,5], history[epoch,6] = val_loss_bbox.mean(), val_loss_seg.mean()
    history[epoch,7], history[epoch,8] = val_bbox_iou.mean(), val_seg_miou.mean()

    if epoch % 5 == 4:
        print('[==============Epoch: %d==============]' % (epoch + 1))
        print('Training: loss_all: %.5f, loss_bbox: %.5f, loss_seg: %.5f' % \
            (history[epoch,0], history[epoch,1], history[epoch,2]))
        print('          iou_bbox: %.5f, miou_seg: %.5f' % \
            (history[epoch,3], history[epoch,4]))
        print('Validation: loss_bbox: %.5f, loss_seg: %.5f' % \
...
torch.save(history, os.path.join(FILE_SAVE_PATH, 'abla_bbox_history.pt'))
torch.save(net.state_dict(), os.path.join(FILE_SAVE_PATH, 'abla_bbox_model.pt'))
torch.save(net, os.path.join(FILE_SAVE_PATH, 'abla_bbox_model_lite.pt'))
...
with torch.no_grad():
    for index, data in enumerate(test_loader, 0):
        ...
        test_bbox_iou[index] = Metrics_IoU(outputs[0], labels_bbox, device).item()
    ...
    print('loss_bbox: %.5f, loss_seg: %.5f' % \
        (test_loss_bbox.mean(), test_loss_seg.mean()))
    print('iou_bbox: %.5f,  miou_seg: %.5f' % \
        (test_bbox_iou.mean(), test_seg_miou.mean()))
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
...
with torch.no_grad():
    for i, data in enumerate(test_loader):
        ...
        pre_bbox = outputs[0][test_index].cpu()
        ...
        plt.savefig(os.path.join(FILE_SAVE_PATH, 'abla_bbox_test.png'))
...
plt.legend()
plt.savefig(os.path.join(FILE_SAVE_PATH, 'abla_bbox_seg_loss.png'))
